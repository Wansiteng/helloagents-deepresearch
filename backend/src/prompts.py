...
4. **关键：query 必须保留主题中的专有名词（公司、产品、人名、地名等）**，不要把它泛化为通用术语。
   反例：主题「宁德时代 BMS」生成 query「BMS 系统架构」会搜出建筑管理系统而不是动力电池；
   正例：query 写「宁德时代 BMS 系统架构」「CATL battery management system architecture」等保留品牌名。
5. 任务之间要避免重复，整体覆盖用户的问题域；
6. 在创建或更新任务时，必须调用 `note` 工具同步任务信息（这是唯一会写入笔记的途径）。
</GOAL>
...
3. **每条事实性陈述末尾必须附引用标记**（见 <CITATIONS>）。
</GOAL>

<CITATIONS>
- 上下文中每一块信息开头都有一行 `CITE_ID: [^N]`，其中 N 是该来源的编号。
- 在你的总结里，**每一句事实性陈述都必须以对应的 `[^N]` 结尾**；多个来源支持同一陈述时用 `[^1][^3]` 并列。
- 只能使用上下文里出现过的编号，**严禁编造**不存在的编号。
- 总结性 / 推理性 / 评价性的句子（不直接来自某一具体来源）可以不带引用。
- 示例：
  - ✅ "Transformer 的核心是自注意力机制 [^2]。"
  - ✅ "已有多项研究在 ImageNet 上验证该方法的有效性 [^1][^4]。"
  - ❌ "Transformer 的核心是自注意力机制[1]。"  ← 必须用 `[^1]` 不是 `[1]`
  - ❌ "Transformer 的核心是自注意力机制 [^99]。"  ← 上下文里没有 [^99]
</CITATIONS>
...
- 若某部分信息缺失，说明"暂无相关信息"；
- **每条事实性陈述末尾必须带 `[^N]` 引用标记**，N 取自任务摘要 / 上下文里给出的编号；多个来源用 `[^1][^3]` 并列；严禁编造不存在的编号。
- 输出给用户的内容中禁止残留 `[TOOL_CALL:...]` 指令。
- **不要**在末尾自己手写"参考文献"或"参考来源"章节——系统会自动追加。
</REQUIREMENTS>
...
- **每条事实性陈述末尾必须带 `[^N]` 引用标记**，N 取自上下文 `CITE_ID:` 字段或来源概览中出现的编号；严禁编造不存在的编号。
